...
if lazy:
  dbold = GraticuleDatabase("lastchangeset.yaml")
  ...
  if data:
    ...
    if text:
      if re_noedit.findall(text):
        ...
        continue
      re_map = re.compile(re_maprough % grat, re.DOTALL)
      ...
      if match:
        ...
        if suggestion == statusquo:
          #print (u'No changes needed on page %s. Yay.' % data[0]).encode("utf-8")
          #confirm = wikipedia.input(u'Continue? [*]> ')
          pass
        else:
